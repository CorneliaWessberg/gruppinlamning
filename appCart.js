    ...
    let products = JSON.parse(localStorage.getItem("cartProducts"))
         ...
         products.map((addedToCart) => {
             ...
             document.querySelector("#products").appendChild(addProductBox)
              addProductBox.innerHTML = `<li class="name" > ${name} </li><li> <img class="imgStyle" src="${img}"> </li>  <li class="description" > ${description} </li> <li class="price"> ${price} </li> <button id="btn"> Remove from cart</button>`
